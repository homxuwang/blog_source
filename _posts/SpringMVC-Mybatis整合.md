...
# 所需依赖包
![所需依赖包](SpringMVC-Mybatis整合/2.png)
![所需依赖包](SpringMVC-Mybatis整合/3.png)

## 配置数据库和逆向工程
数据库表:
![items表](SpringMVC-Mybatis整合/5.png)
![user表](SpringMVC-Mybatis整合/6.png)
使用逆向工程生成pojo和dao:
![pojo和dao](SpringMVC-Mybatis整合/4.png)
...
```
![ContextLoaderListener的位置](SpringMVC-Mybatis整合/1.png)
配置至此，如果开启tomcat,那么就可以连接数据库了

## 配置事务
![配置注解事务](SpringMVC-Mybatis整合/7.png)
...
# 从mysql中读取信息并显示
书写service层代码:
![service层](SpringMVC-Mybatis整合/8.png)
`ItemService.java`:
```java
...
```
![测试结果](SpringMVC-Mybatis整合/9.png)
